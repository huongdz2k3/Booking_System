...
func (svc *CustomerService) Register(ctx context.Context, req *RegisterInput) (*Customer, error) {
	...
	checkExist, _ := svc.GetCustomerByEmail(ctx, &GetCustomerByEmailInput{Email: req.GetEmail()})
	...
	u := svc.client.Customer.Create().SetName(req.GetName()).SetEmail(req.GetEmail()).SetPhoneNumber(req.GetPhoneNumber()).SetLicenseID(req.GetLicenseId()).SetAddress(req.GetAddress()).SetMembershipNumber(int(req.GetMembershipNumber())).SetPassword(req.GetPassword()).SetRole("SUBSCRIBER").SaveX(ctx)
...
func (svc *CustomerService) Login(ctx context.Context, req *LoginInput) (*Customer, error) {
	...
	cus, _ := svc.GetCustomerByEmail(ctx, &GetCustomerByEmailInput{Email: req.GetEmail()})
